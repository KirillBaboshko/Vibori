...
int main()
{
	...
	date myBirthday;
	const int n = 3;                                                          // берем 10 , хотя можно и другое число
	int day;
	int mounth;
	int yers;
	...
	/*if ((out = fopen("D:\\example\\spisok.txt", "w")) == NULL)
	{
		cout << "Fale uge esti";
	}
	...
	const char* path = "D:\\example\\spisok.txt";
	...
	else
	{
		for (int i = 0; i < n; i++)
		{
			fprintf(out, "Избиратель №");
			fprintf(out, "%ld", i+1);
			fprintf(out, "\n");
			fprintf(out, "%s", spisok[i].Neme);
			fprintf(out, " ");
			fprintf(out, "%s", spisok[i].Surneme);
			fprintf(out, " ");
			fprintf(out, "%s", spisok[i].Otchestvo);
			fprintf(out, "\n");
			fprintf(out, "%ld", spisok[i].myBirthday.day);
			fprintf(out, ".");
			fprintf(out, "%ld", spisok[i].myBirthday.month);
			fprintf(out, ".");
			fprintf(out, "%ld", spisok[i].myBirthday.year);
			fprintf(out, "\n");
		}
		if (fclose(out) == EOF)
		{
